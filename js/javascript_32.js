...
const solutionFirst = (nums) => {
  const max = nums.length / 2; // 최대 가지고 갈 수 있는 포켓몬 수 2
  const arr = [...new Set(nums)]; // 포켓몬 종류 3
...
/*
  # 문제풀이 - solutionFirst
  1. 변수에 가지고 갈 수 있는 폰켓몬 갯수를 담아둔다. 폰켓몬을 N/2만큼 가지고 갈 수 있으므로 최대 갯수는 파라미터/2가 된다.
  2. Set 생성자를 통해 중복 폰켓몬 종류를 제외하고 가지고 갈 수 있는 폰켓몬 종류를 구한다.
  3. 삼항연산자를 활용하여 총 포켓몬 종류가 가지고 갈 수 있는 갯수보다 크다면 최대 갯수를 출력하고 아닌 경우 포켓몬 종류를 출력한다.
*/
